...
module.exports = { getDeps, findModules, findOneModule };

/**
 * @typedef {import('./js-doc.js').SearchOptions} SearchOptions
 */
...
async function findOneModule(name, version) {
  ...
  try {
    if (version) {
      ...
      response = await customFetch(`https://registry.npmjs.org/${name}`);
    }
    ...
    if (error.response.status === 404) {
      // 404 + version => sth wrong w/ version
      if (version) {
        // retry w/o version here OR in getDeps' catch
        console.log("Retry w/o version ");
        retried = true;
        await findOneModule(name);

        //404 w/o version - package DNE
      } else {
